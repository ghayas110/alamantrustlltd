...
export default function CookieConsent() {
  ...
  const handleAcceptAll = () => {
    ...
    localStorage.setItem("cookie-consent", JSON.stringify({ decision: "accepted", preferences: all, timestamp: new Date().toISOString() }));
    dismiss();
  };

  const handleDecline = () => {
    localStorage.setItem("cookie-consent", JSON.stringify({ decision: "declined", preferences: DEFAULT_PREFERENCES, timestamp: new Date().toISOString() }));
    dismiss();
  };

  const handleSavePreferences = () => {
    localStorage.setItem("cookie-consent", JSON.stringify({ decision: "custom", preferences, timestamp: new Date().toISOString() }));
    dismiss();
  };
